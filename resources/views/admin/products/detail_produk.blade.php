...
<html lang="id" class="h-full">
  ...
  <body class="bg-[#F8FFF9] text-gray-700 font-sans flex">
    ...
    <select id="statusSelect" name="kurasi_status" class="w-full border rounded p-2 mb-4" required>
        <option value="diterima" @selected($product->kurasi_status === 'diterima')>Diterima</option>
        <option value="ditolak" @selected($product->kurasi_status === 'ditolak')>Ditolak</option>
        <option value="revisi" @selected($product->kurasi_status === 'revisi')>Revisi</option>
    </select>

    <div id="notesContainer" class="hidden mb-4">
        <label for="notes" class="block mb-2 font-semibold">Catatan Revisi:</label>
        <textarea id="notes" name="kurasi_notes" class="w-full border rounded p-2" rows="3">{{ old('kurasi_notes', $product->kurasi_notes ?? '') }}</textarea>
    </div>
    ...
    <script>
        ...
        function toggleNotes() {
            if (statusSelect.value.toLowerCase() === 'revisi') {
        ...
        // Event listener saat opsi status berubah
